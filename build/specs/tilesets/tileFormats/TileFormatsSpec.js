...
describe("TileFormats", function () {
    it("reads B3DM (deprecated 1) from a buffer", function () {
        const p = SPECS_DATA_BASE_DIRECTORY + "/BatchedDeprecated1/batchedDeprecated1.b3dm";
        const tileDataBuffer = fs_1.default.readFileSync(p);
        const tileData = tilesets_1.TileFormats.readTileData(tileDataBuffer);
    ...
    it("reads CMPT from a buffer", function () {
        ...
        const tileDataBuffer = fs_1.default.readFileSync(p);
        const tileData = tilesets_1.TileFormats.readCompositeTileData(tileDataBuffer);
    ...
    it("extracts a single GLB buffers from B3DM", async function () {
        ...
        const glbBuffers = await tilesets_1.TileFormats.extractGlbBuffers(tileDataBuffer, externalGlbResolver);
    ...
    it("extracts a single GLB buffer from an I3DM that refers to an external GLB", async function () {
        const p = SPECS_DATA_BASE_DIRECTORY + "/tileFormats/instancedGltfExternal.i3dm";
        const tileDataBuffer = fs_1.default.readFileSync(p);
        const externalGlbResolver = createResolver(p);
        const glbBuffers = await tilesets_1.TileFormats.extractGlbBuffers(tileDataBuffer, externalGlbResolver);
        expect(glbBuffers.length).toBe(1);
    ...
    it("extracts a single GLB buffer from an I3DM that refers to an external GLB with spaces as padding bytes", async function () {
        const p = SPECS_DATA_BASE_DIRECTORY +
    ...
    it("properly omits padding bytes in extractGlbPayload for b3dmToGlb", async function () {
        ...
        const inputGlbBuffer = fs_1.default.readFileSync(p);
        const inputB3dmTileData = tilesets_1.TileFormats.createDefaultB3dmTileDataFromGlb(inputGlbBuffer);
        const b3dmBuffer = tilesets_1.TileFormats.createTileDataBuffer(inputB3dmTileData);
        const outputB3dmTileData = tilesets_1.TileFormats.readTileData(b3dmBuffer);
        const outputGlbBuffer = tilesets_1.TileFormats.extractGlbPayload(outputB3dmTileData);
    ...
    it("splits a composite with splitCmpt", async function () {
        ...
        const inputBuffer = fs_1.default.readFileSync(p);
    ...
    it("throws an error when trying to read tile data from a buffer that does not contain B3DM, I3DM, or PNTS", function () {
        ...
        const tileDataBuffer = fs_1.default.readFileSync(p);
        expect(function () {
            tilesets_1.TileFormats.readTileData(tileDataBuffer);
        }).toThrowError();
    });
    it("throws an error when trying to read composite tile data from a buffer that does not contain CMPT", function () {
        ...
        const tileDataBuffer = fs_1.default.readFileSync(p);
        expect(function () {
            tilesets_1.TileFormats.readCompositeTileData(tileDataBuffer);
        }).toThrowError();
    });
    it("creates the required default feature table JSON for B3DM", function () {
        const glbBuffer = Buffer.alloc(10);
        const tileData = tilesets_1.TileFormats.createB3dmTileDataFromGlb(glbBuffer, undefined, undefined, undefined, undefined);
        expect(tileData.featureTable.json).toBeDefined();
        expect(tileData.featureTable.json.BATCH_LENGTH).toBe(0);
    });
    it("creates the required default feature table for I3DM", function () {
        const glbBuffer = Buffer.alloc(10);
        const tileData = tilesets_1.TileFormats.createI3dmTileDataFromGlb(glbBuffer, undefined, undefined, undefined, undefined);
        expect(tileData.featureTable.json).toBeDefined();
    ...
    it("creates a valid buffer from B3DM tile data", function () {
        const glbBuffer = Buffer.alloc(10);
        const tileData = tilesets_1.TileFormats.createDefaultB3dmTileDataFromGlb(glbBuffer);
        const buffer = tilesets_1.TileFormats.createTileDataBuffer(tileData);
    ...
    it("creates a valid buffer from I3DM tile data", function () {
        const glbBuffer = Buffer.alloc(10);
        const tileData = tilesets_1.TileFormats.createDefaultI3dmTileDataFromGlb(glbBuffer);
        const buffer = tilesets_1.TileFormats.createTileDataBuffer(tileData);
        // Note: There are some assumptions here about the exact
        // format of the default feature table JSON that is
        // inserted. It COULD contain an arbitrary number of
        ...
        expect(buffer.readUInt32LE(8)).toEqual(expectedByteLength); // byteLength
        expect(buffer.readUInt32LE(12)).toEqual(expectedFeatureTableJsonByteLength); // featureTableJSONByteLength
        expect(buffer.readUInt32LE(16)).toEqual(expectedFeatureTableBinaryByteLength); // featureTableBinaryByteLength
        expect(buffer.readUInt32LE(20)).toEqual(0); // batchTableJSONByteLength
        expect(buffer.readUInt32LE(24)).toEqual(0); // batchTableBinaryByteLength
    ...
    it("creates a valid buffer from CMPT tile data", function () {
        const glbBuffer = Buffer.alloc(10);
        const b3dmTileData = tilesets_1.TileFormats.createDefaultB3dmTileDataFromGlb(glbBuffer);
        const i3dmTileData = tilesets_1.TileFormats.createDefaultI3dmTileDataFromGlb(glbBuffer);
        const b3dmBuffer = tilesets_1.TileFormats.createTileDataBuffer(b3dmTileData);
        const i3dmBuffer = tilesets_1.TileFormats.createTileDataBuffer(i3dmTileData);
        const b3dmOriginalLength = b3dmBuffer.length;
        const i3dmOriginalLength = i3dmBuffer.length;
        const cmptTileData = tilesets_1.TileFormats.createCompositeTileData([
            b3dmTileData,
            i3dmTileData,
        ]);
        const cmpt = tilesets_1.TileFormats.createCompositeTileDataBuffer(cmptTileData);
        const magic = base_1.Buffers.getMagicString(cmpt);
        ...
        const expectedByteLength = headerByteLength +
            SpecHelpers_1.SpecHelpers.getPaddedByteLength(b3dmOriginalLength) +
            SpecHelpers_1.SpecHelpers.getPaddedByteLength(i3dmOriginalLength);
        ...
        const b3dmMagic = base_1.Buffers.getMagicString(cmpt, headerByteLength);
        ...
        const i3dmMagic = base_1.Buffers.getMagicString(cmpt, headerByteLength + b3dmByteLength);
    ...
    it("extracts a b3dm from buffer", function () {
        ...
        const b3dmPath = SPECS_DATA_BASE_DIRECTORY + "/batchedWithBatchTableBinary.b3dm";
        const b3dmBuffer = fs_1.default.readFileSync(b3dmPath);
        const b3dm = tilesets_1.TileFormats.readTileData(b3dmBuffer);
    ...
    it("extracts a b3dm and i3dm from composite buffer", function () {
        ...
        const compositeBuffer = fs_1.default.readFileSync(compositePath);
        const compositeTileData = tilesets_1.TileFormats.readCompositeTileData(compositeBuffer);
    ...
    it("extracts a b3dm and i3dm from composite-of-composite buffer", function () {
        // Ported from legacy tests
        const compositeOfCompositePath = SPECS_DATA_BASE_DIRECTORY + "/compositeOfComposite.cmpt";
        const compositeOfCompositeBuffer = fs_1.default.readFileSync(compositeOfCompositePath);
        const compositeOfCompositeTileData = tilesets_1.TileFormats.readCompositeTileData(compositeOfCompositeBuffer);
        const innerCompositeBuffer = compositeOfCompositeTileData.innerTileBuffers[0];
        const compositeTileData = tilesets_1.TileFormats.readCompositeTileData(innerCompositeBuffer);
        const innerTiles = compositeTileData.innerTileBuffers;
        const b3dmMagic = innerTiles[0].toString("utf8", 0, 4);
        const i3dmMagic = innerTiles[1].toString("utf8", 0, 4);
        expect(innerTiles.length).toBe(2);
    ...
    it("extracts a i3dm from buffer", function () {
        // Ported from legacy tests
        const i3dmPath = SPECS_DATA_BASE_DIRECTORY + "/instancedWithBatchTableBinary.i3dm";
        const i3dmBuffer = fs_1.default.readFileSync(i3dmPath);
        const i3dm = tilesets_1.TileFormats.readTileData(i3dmBuffer);
    ...
    it("generates a basic b3dm header for a glb", function () {
        // Ported from legacy tests
        const glbPath = SPECS_DATA_BASE_DIRECTORY + "/CesiumTexturedBox/CesiumTexturedBox.glb";
        const glbBuffer = fs_1.default.readFileSync(glbPath);
    ...
    it("generates a b3dm with feature table and batch table", function () {
        ...
        const glbPath = SPECS_DATA_BASE_DIRECTORY + "/CesiumTexturedBox/CesiumTexturedBox.glb";
        const glbBuffer = fs_1.default.readFileSync(glbPath);
        const tileData = tilesets_1.TileFormats.createB3dmTileDataFromGlb(glbBuffer, featureTableJson, featureTableBinaryBuffer, batchTableJson, batchTableBinaryBuffer);
        const b3dmBuffer = tilesets_1.TileFormats.createTileDataBuffer(tileData);
        const headerByteLength = 28;
        const header = b3dmBuffer.subarray(0, headerByteLength);
        expect(header.toString("utf8", 0, 4)).toEqual("b3dm"); // magic
        expect(header.readUInt32LE(4)).toEqual(1); // version
    ...
    it("generates a basic i3dm header for a glb", function () {
        ...
        const tileData = tilesets_1.TileFormats.createDefaultI3dmTileDataFromGlb(glbBuffer);
        const i3dmBuffer = tilesets_1.TileFormats.createTileDataBuffer(tileData);
    ...
    it("generates an i3dm with feature table and batch table", function () {
        ...
        const featureTableBinaryBuffer = Buffer.alloc(16); // Contents don't matter
        const batchTableBinaryBuffer = Buffer.alloc(32); // Contents don't matter
        const tileData = tilesets_1.TileFormats.createI3dmTileDataFromGlb(glbBuffer, featureTableJson, featureTableBinaryBuffer, batchTableJson, batchTableBinaryBuffer);
        const i3dmBuffer = tilesets_1.TileFormats.createTileDataBuffer(tileData);
        const headerByteLength = 32;
        const header = i3dmBuffer.subarray(0, headerByteLength);
        expect(header.toString("utf8", 0, 4)).toEqual("i3dm"); // magic
...
//# sourceMappingURL=TileFormatsSpec.js.map