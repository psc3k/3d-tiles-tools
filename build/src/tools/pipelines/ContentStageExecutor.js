...
const logger = base_3.Loggers.get("pipeline");
/**
...
class ContentStageExecutor {
    ...
    static async executeGlbToB3dm(tilesetProcessor) {
        ...
        const uriProcessor = (uri) => {
            if (base_1.Paths.hasExtension(uri, ".glb")) {
                return base_1.Paths.replaceExtension(uri, ".b3dm");
            }
            return uri;
        };
        ...
        const entryProcessor = async (sourceEntry, type) => {
            if (type !== base_2.ContentDataTypes.CONTENT_TYPE_GLB) {
                return sourceEntry;
            }
            const targetEntry = {
    ...
    static async executeGlbToI3dm(tilesetProcessor) {
        ...
        const uriProcessor = (uri) => {
            if (base_1.Paths.hasExtension(uri, ".glb")) {
                return base_1.Paths.replaceExtension(uri, ".i3dm");
            }
            return uri;
        };
        // Define the `TilesetEntryProcessor` that generates an
    ...
    static async executeB3dmToGlb(tilesetProcessor) {
        ...
        const entryProcessor = async (sourceEntry, type) => {
            if (type !== base_2.ContentDataTypes.CONTENT_TYPE_B3DM) {
                return sourceEntry;
            }
            const targetEntry = {
                key: uriProcessor(sourceEntry.key),
    ...
    static async executeI3dmToGlb(tilesetProcessor) {
        ...
        const entryProcessor = async (sourceEntry, type) => {
            if (type !== base_2.ContentDataTypes.CONTENT_TYPE_I3DM) {
                return sourceEntry;
            }
            const targetEntry = {
                key: uriProcessor(sourceEntry.key),
    ...
    static async executeConvertB3dmToGlb(tilesetProcessor) {
        ...
        const entryProcessor = async (sourceEntry, type) => {
            ...
            const gltfUpAxis = "Y";
            const targetValue = await TileFormatsMigration_1.TileFormatsMigration.convertB3dmToGlb(sourceEntry.value, gltfUpAxis);
            const targetEntry = {
                key: uriProcessor(sourceEntry.key),
                value: targetValue,
            };
    ...
    static async executeConvertPntsToGlb(tilesetProcessor) {
        ...
        const entryProcessor = async (sourceEntry, type) => {
            if (type !== base_2.ContentDataTypes.CONTENT_TYPE_PNTS) {
                return sourceEntry;
            }
    ...
    static async executeSeparateGltf(tilesetProcessor) {
        ...
        const uriProcessor = (uri) => {
            if (base_1.Paths.hasExtension(uri, ".glb")) {
                return base_1.Paths.replaceExtension(uri, ".gltf");
            }
            return uri;
        };
        // The entry processor receives the source entry, and
        ...
        const entryProcessor = async (sourceEntry, type) => {
            ...
            const prefix = base_1.Paths.replaceExtension(path_1.default.basename(sourceEntry.key), "");
            const options = {
                separate: true,
                name: prefix,
            };
            ...
            for (const resourceKey of resourceKeys) {
                ...
                const resourceTargetEntry = {
                    key: base_1.Paths.join(dirname, resourceKey),
                    value: resourceValue,
                };
...
//# sourceMappingURL=ContentStageExecutor.js.map