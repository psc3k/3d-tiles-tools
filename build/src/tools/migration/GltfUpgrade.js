...
class GltfUpgrade {
     ...
     */
    static async obtainDocument(glb, gltfUpAxis) {
        const gltfVersion = GltfUtilities_1.GltfUtilities.getGltfVersion(glb);
        if (gltfVersion < 2.0) {
            logger.info("Found glTF 1.0 - upgrading to glTF 2.0 with gltf-pipeline");
            // Upgrade the GLB buffer to glTF 2.0 if necessary.
            glb = await GltfUtilities_1.GltfUtilities.upgradeGlb(glb, undefined);
        }
        ...
        glb = GltfUtilities_1.GltfUtilities.replaceCesiumRtcExtensionInGltf2Glb(glb, gltfUpAxis);
        if (gltfVersion < 2.0) {
            ...
            // document internally)
            glb = await GltfUtilities_1.GltfUtilities.replaceWeb3dQuantizedAttributesExtension(glb);
        }
        ...
        if (gltfVersion < 2.0) {
            ...
            document.setLogger(new core_1.Logger(core_1.Logger.Verbosity.WARN));
            await document.transform((0, functions_1.prune)());
        }
        // Apply the up-axis conversion to the resulting document
        TileFormatsMigration_1.TileFormatsMigration.applyGltfUpAxis(document, gltfUpAxis);
        return document;
    }
    /**
     ...
     */
    static octDecode2DNormals(document) {
        const root = document.getRoot();
            ...
            for (const primitive of primitives) {
                ...
                if (normalAccessor) {
                    ...
                    if (type === "VEC2" && componentType === GL_BYTE) {
                        ...
                        const decodedNormalsAccessor = GltfUpgrade.octDecodeAccessor(document, normalAccessor, 255.0);
                        ...
                        const decodedNormalsAccessor = GltfUpgrade.octDecodeAccessor(document, normalAccessor, 65535.0);
                        primitive.setAttribute("NORMAL", decodedNormalsAccessor);
                        decodedAccessorsCounter++;
                    }
                }
     ...
     */
    static octDecodeAccessor(document, encodedAccessor, range) {
        const decodedData = [];
     ...
     */
    static octDecode(encoded, range) {
     ...
     * @param document - The glTF-Transform document
     */
    static decode3DTexCoords(document) {
        const root = document.getRoot();
        const meshes = root.listMeshes();
        let decodedAccessorsCounter = 0;
        for (const mesh of meshes) {
                    ...
                    if (semantic.startsWith("TEXCOORD_")) {
                        ...
                        if (texcoordAccessor) {
                            ...
                            if (type === "VEC3" && componentType === GL_BYTE) {
                                ...
                                const decodedTexCoordsAccessor = GltfUpgrade.decodeTexCoordAccessor(document, texcoordAccessor, 127.0);
                                ...
                                const decodedTexCoordsAccessor = GltfUpgrade.decodeTexCoordAccessor(document, texcoordAccessor, 32767.0);
                                primitive.setAttribute(semantic, decodedTexCoordsAccessor);
                                decodedAccessorsCounter++;
                            }
                        }
        ...
        if (decodedAccessorsCounter > 0) {
            logger.info(`Decoded ${decodedAccessorsCounter} texture coordinate accessors to 2D`);
        }
    }
    /**
     ...
     */
    static decodeTexCoordAccessor(document, encodedAccessor, range) {
        const decodedData = [];
        const count = encodedAccessor.getCount();
        for (let i = 0; i < count; i++) {
            const encoded = [0, 0, 0];
     ...
     */
    static decodeTexCoord(encoded, range) {
...
exports.GltfUpgrade = GltfUpgrade;
//# sourceMappingURL=GltfUpgrade.js.map