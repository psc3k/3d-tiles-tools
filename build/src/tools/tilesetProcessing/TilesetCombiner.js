...
const logger = base_4.Loggers.get("tilesetProcessing");
/**
...
class TilesetCombiner {
    ...
    async combine(tilesetSourceName, tilesetTargetName, overwrite) {
        ...
        const tilesetSourceJsonFileName = tilesets_5.Tilesets.determineTilesetJsonFileName(tilesetSourceName);
        const tilesetTargetJsonFileName = tilesets_5.Tilesets.determineTilesetJsonFileName(tilesetTargetName);
        await this.combineData(tilesetSource, tilesetSourceJsonFileName, tilesetTarget, tilesetTargetJsonFileName);
     ...
     * or when the output already exists and `overwrite` was `false`.
     */
    async combineData(tilesetSource, tilesetSourceJsonFileName, tilesetTarget, tilesetTargetJsonFileName) {
    ...
    async combineInternal(tilesetSource, tilesetSourceJsonFileName, tilesetTarget, tilesetTargetJsonFileName) {
        ...
        await tilesetTarget.addEntry(tilesetTargetJsonFileName, combinedTilesetJsonBuffer);
    }
    /**
     ...
     */
    async combineTilesetsInternal(currentDirectory, tileset) {
     ...
     */
    async processExternalTileset(externalFileName, externalFileBuffer) {
        ...
        const externalTileset = JSON.parse(externalFileBuffer.toString());
        await this.combineTilesetsInternal(externalTilesetDirectory, externalTileset);
     ...
     * @param tile - The current tile
     */
    async combineSingleContentInternal(currentDirectory, tile) {
        if (!this.tilesetSource || !this.tilesetTarget) {
            throw new base_2.DeveloperError("The source and target must be defined");
        }
        const content = tile.content;
        if (!content) {
            throw new base_2.DeveloperError("This should only be called for tiles with a single content");
        }
        // Obtain the data of the content, and determine whether it is
        // an external tileset
        const contentUri = TilesetCombiner.obtainContentUri(content);
        const externalFileName = base_1.Paths.join(currentDirectory, contentUri);
        const externalFileBuffer = await this.tilesetSource.getValue(externalFileName);
        if (!externalFileBuffer) {
            throw new tilesets_1.TilesetError(`No data found for ${externalFileName}`);
        }
        const contentData = new base_3.BufferedContentData(contentUri, externalFileBuffer);
        ...
        if (!isTileset) {
            ...
            // will end up in
            const newUri = base_1.Paths.relativize(".", externalFileName);
            content.uri = newUri;
            return;
        }
        // The data is an external tileset. Process (combine) this
        // tileset, recursively
        const externalTileset = await this.processExternalTileset(externalFileName, externalFileBuffer);
     ...
     * @param tile - The current tile
     */
    async combineMultipleContentsInternal(currentDirectory, tile) {
        if (!this.tilesetSource || !this.tilesetTarget) {
            throw new base_2.DeveloperError("The source and target must be defined");
        }
        const contents = tile.contents;
        if (!contents) {
            throw new base_2.DeveloperError("This should only be called for tiles with multiple contents");
        }
        ...
        tilesets_4.Tiles.setContents(tile, newContents);
     ...
     */
    static obtainContentUri(content) {
        ...
        if (legacyContent.url === undefined) {
            ...
            throw new tilesets_1.TilesetError("Content does neither contain a 'uri' nor a (legacy) 'url' property");
        }
        // Remove the legacy content.url, and set the URL
        // as the content.uri instead
        logger.warn("The 'url' property of tile content is deprecated. Using 'uri' in combined result.");
     ...
     */
    async copyResources(tilesetTargetJsonFileName) {
        if (!this.tilesetSource || !this.tilesetTarget) {
            throw new base_2.DeveloperError("The source and target must be defined");
        }
        const entries = await tilesets_2.TilesetSources.getEntries(this.tilesetSource);
...
//# sourceMappingURL=TilesetCombiner.js.map