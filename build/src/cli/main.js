...
let logger = base_2.Loggers.get("CLI");
...
function parseToolArgs(a) {
    const args = (0, yargs_1.default)(a)
        ...
        .command("splitCmpt", "Split the input cmpt and write out its inner tiles.", {
        i: inputStringDefinition,
        o: outputStringDefinition,
        recursive: {
            default: false,
        ...
        .command("updateAlignment", "Update the alignment of the batch- and feature table and the tile data" +
        ...
        .command("mergeJson", "Merge any number of tilesets together into a single tileset without copying resources to output directory.", {
        i: inputArrayDefinition,
        o: outputStringDefinition,
    })
        ...
        .command("createTilesetJson", "Creates a tileset JSON file that just refers to given tile content files. " +
        ...
        cartographicPositionDegrees: {
            description: "An array of either two or three values, which are the (longitude, latitude) " +
        ...
        rotationDegrees: {
            description: "An array of three values, which are the (heading, pitch, roll) " +
 ...
 * constraints.
 */
function validateOptionalNumberArray(value, minLength, maxLength) {
    if (value === undefined) {
        return undefined;
    }
    if (!Array.isArray(value)) {
        throw new base_1.DeveloperError(`Expected an array, but received ${value}`);
    }
    if (minLength !== undefined) {
        if (value.length < minLength) {
            throw new base_1.DeveloperError(`Expected an array of at least length ${minLength}, ` +
                `but received an array of length ${value.length}: ${value}`);
        }
    }
    if (maxLength !== undefined) {
        if (value.length > maxLength) {
            throw new base_1.DeveloperError(`Expected an array of at most length ${maxLength}, ` +
                `but received an array of length ${value.length}: ${value}`);
        }
    }
    for (let i = 0; i < value.length; i++) {
        ...
        if (type !== "number") {
            throw new base_1.DeveloperError(`Expected an array of numbers, but element at index ${i} ` +
...
async function run() {
    ...
    if (logJson === true) {
        ...
        base_2.Loggers.initDefaultLogger(prettyPrint);
        logger = base_2.Loggers.get("CLI");
    }
    ...
    if (logLevel !== undefined) {
        ...
        if (validLogLevels.includes(logLevel)) {
            base_2.Loggers.setLevel(logLevel);
        }
        else {
            logger.warn(`Invalid log level: ${logLevel}`);
            base_2.Loggers.setLevel("info");
        }
    }
...
async function runCommand(command, toolArgs, optionArgs) {
    ...
    if (command === "b3dmToGlb") {
        ...
        await ToolsMain_1.ToolsMain.splitCmpt(input, output, recursive, force);
    }
        ...
        await ToolsMain_1.ToolsMain.updateAlignment(input, output, force);
    }
        ...
        await ToolsMain_1.ToolsMain.upgrade(input, output, force, targetVersion, parsedOptionArgs);
    }
    else if (command === "merge") {
        await ToolsMain_1.ToolsMain.merge(inputs, output, force);
    }
    else if (command === "mergeJson") {
        await ToolsMain_1.ToolsMain.mergeJson(inputs, output, force);
    }
        ...
        const cartographicPositionDegrees = validateOptionalNumberArray(toolArgs.cartographicPositionDegrees, 2, 3);
        const rotationDegrees = validateOptionalNumberArray(toolArgs.rotationDegrees, 3, 3);
        await ToolsMain_1.ToolsMain.createTilesetJson(input, output, cartographicPositionDegrees, rotationDegrees, force);
    }
    else {
        throw new base_1.DeveloperError(`Invalid command: ${command}`);
    }
}
