...
class TileFormats {
    ...
    static isComposite(buffer) {
        const magic = base_1.Buffers.getMagicString(buffer);
        return magic === "cmpt";
    }
    /**
    ...
    static extractTileData(buffer, tileDataLayout) {
        ...
        let featureTableJson = base_1.Buffers.getJson(featureTableJsonBuffer);
        const batchTableJson = base_1.Buffers.getJson(batchTableJsonBuffer);
     ...
     */
    static async extractGlbBuffers(tileDataBuffer, externalGlbResolver) {
        const glbBuffers = [];
        await TileFormats.extractGlbBuffersInternal(tileDataBuffer, externalGlbResolver, glbBuffers);
        return glbBuffers;
    }
    /**
     ...
     */
    static async extractGlbBuffersInternal(tileDataBuffer, externalGlbResolver, glbBuffers) {
        ...
        if (isComposite) {
            ...
            for (const innerTileDataBuffer of compositeTileData.innerTileBuffers) {
                await TileFormats.extractGlbBuffersInternal(innerTileDataBuffer, externalGlbResolver, glbBuffers);
            }
        }
        else {
            ...
            if (tileData.header.magic === "b3dm" ||
                ...
                const glbPayload = await TileFormats.obtainGlbPayload(tileData, externalGlbResolver);
                if (glbPayload) {
     ...
     */
    static async splitCmpt(tileDataBuffer, recursive) {
        const resultBuffers = [];
        await TileFormats.splitCmptInternal(tileDataBuffer, recursive, resultBuffers);
     ...
     */
    static async splitCmptInternal(tileDataBuffer, recursive, resultBuffers) {
        const isComposite = TileFormats.isComposite(tileDataBuffer);
        if (!isComposite) {
            ...
            if (!recursive) {
                ...
                for (const innerTileBuffer of compositeTileData.innerTileBuffers) {
                    await TileFormats.splitCmptInternal(innerTileBuffer, recursive, resultBuffers);
                }
            }
        }
    }
    ...
    static createTileDataBuffer(tileData) {
        ...
        const featureTableJsonBuffer = base_1.Buffers.getJsonBufferPadded(featureTable.json, headerByteLength);
        const featureTableBinaryBuffer = base_1.Buffers.getBufferPadded(featureTable.binary);
        const batchTableJsonBuffer = base_1.Buffers.getJsonBufferPadded(batchTable.json);
        const batchTableBinaryBuffer = base_1.Buffers.getBufferPadded(batchTable.binary);
        const payload = base_1.Buffers.getBufferPadded(tileData.payload);
    ...
    static createCompositeTileDataBuffer(compositeTileData) {
        ...
        for (let i = 0; i < tilesLength; i++) {
            ...
            const innerBufferPadded = base_1.Buffers.getBufferPadded(innerBuffer, byteLength);
     ...
     */
    static extractGlbPayload(tileData) {
     ...
     */
    static async obtainGlbPayload(tileData, externalGlbResolver) {
     ...
     */
    static stripGlbPaddingBytes(input) {
...
//# sourceMappingURL=TileFormats.js.map