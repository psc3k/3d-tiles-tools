...
class SubtreeModels {
    ...
    static async resolve(implicitTiling, schema, resourceResolver, coordinates) {
        ...
        if (!(0, base_1.defined)(subtreeUri)) {
        ...
        const isJson = base_2.Buffers.isProbablyJson(subtreeData);
        if (isJson) {
            let subtreeJson;
            let subtree;
            try {
                subtreeJson = base_2.Buffers.getJson(subtreeData);
                subtree = subtreeJson;
            }
            catch (error) {
        ...
        const isSubt = base_2.Buffers.getMagicString(subtreeData) === "subt";
        if (isSubt) {
...
//# sourceMappingURL=SubtreeModels.js.map