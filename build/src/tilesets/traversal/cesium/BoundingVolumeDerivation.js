...
class BoundingVolumeDerivation {
    ...
    static deriveBoundingBox(rootBox, level, x, y, z) {
        ...
        const rootHalfAxes = cesium_3.Matrix3.unpack(rootBox, 3, BoundingVolumeDerivation.scratchHalfAxes);
        ...
        center = cesium_3.Matrix3.multiplyByVector(rootHalfAxes, center, BoundingVolumeDerivation.scratchCenter);
        center = cesium_1.Cartesian3.add(center, rootCenter, BoundingVolumeDerivation.scratchCenter);
        let halfAxes = cesium_3.Matrix3.clone(rootHalfAxes);
        halfAxes = cesium_3.Matrix3.multiplyByScale(halfAxes, scaleFactors, halfAxes);
        const childBox = Array(12);
        cesium_1.Cartesian3.pack(center, childBox);
        cesium_3.Matrix3.pack(halfAxes, childBox, 3);
        return childBox;
    }
    /**
     * Derive a bounding volume for a descendant tile (child, grandchild, etc.),
    ...
    static deriveBoundingRegion(rootRegion, level, x, y, z) {
        ...
        const rectangle = cesium_4.Rectangle.unpack(rootRegion, 0, BoundingVolumeDerivation.scratchRectangle);
        const rootMinimumHeight = rootRegion[4];
        const rootMaximumHeight = rootRegion[5];
        const tileScale = Math.pow(2, -level);
        const childWidth = tileScale * rectangle.width;
        const west = cesium_2.Math.negativePiToPi(rectangle.west + x * childWidth);
        const east = cesium_2.Math.negativePiToPi(west + childWidth);
        const childHeight = tileScale * rectangle.height;
        const south = cesium_2.Math.negativePiToPi(rectangle.south + y * childHeight);
        const north = cesium_2.Math.negativePiToPi(south + childHeight);
...
BoundingVolumeDerivation.scratchHalfAxes = new cesium_3.Matrix3();
BoundingVolumeDerivation.scratchRectangle = new cesium_4.Rectangle();
//# sourceMappingURL=BoundingVolumeDerivation.js.map