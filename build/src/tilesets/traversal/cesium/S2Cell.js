...
class S2Cell {
    ...
    static fromFacePositionLevel(face, position, level) {
        if (face < 0 || face > 5) {
            throw new base_1.DeveloperError("Invalid S2 Face (must be within 0-5)");
        }
        if (level < 0 || level > S2Cell.S2_MAX_LEVEL) {
            throw new base_1.DeveloperError("Invalid level (must be within 0-30)");
        }
        if (position < 0 || position >= Math.pow(4, level)) {
            throw new base_1.DeveloperError("Invalid Hilbert position for level");
        }
